...
class Velocity(Constants):
    ...
    """
    def __init__(self, in_temp=60, dt=1, DS_flow_rate=0.05257, DS_gas_flow_rate=0.05257) -> None:
        super().__init__(in_temp, dt, DS_flow_rate)
    ...
    def calc_DS_liquid_sup_velocity(self,cell):              #does not take into account friction
        area=self.topology['DS areas'][cell]
        if area <= 0:
    ...
    def calc_AN_liquid_sup_velocity(self,cell):              #does not take into account friction or gravity or 
        area=self.topology['AN areas'][cell]
        if area <= 0:
            raise Exception('Area cannot be zero or negative')
        velocity=self.q/area
...
V=Velocity()
V.calc_sup_velocities()
#print(len(V.liquid_sup_velocities['DS superficial velocities']))   
x = [n*100 for n in range(50)]
y=V.liquid_sup_velocities['DS superficial velocities']
z=V.liquid_sup_velocities['AN superficial velocities']
plt.plot(y,x,label='drill string')
plt.plot(z,x,label='annulus')
plt.legend()
plt.ylabel('depth (ft)')
plt.gca().invert_yaxis()
plt.xlabel('velovity (m/s)')
